...
3. Cross-process stability depends on the data structure:
   - ordered inputs such as lists, tuples, dictionaries with fixed insertion order, recursive containers, and the fixed-filename code object stayed stable across tested subprocesses;
   - string-based `set` and `frozenset` specimens produced different hashes under different `PYTHONHASHSEED` values.
4. The round-trip property held for the valid specimens, including recursive containers and shared references.
5. Lexical fuzzing generated 30 mutated byte streams:
...
6. Generation-based fuzzing found no local counterexample in the configured run budget.

Interpretation:

- The strongest local stability risk is unordered string-hash-based containers across subprocesses.
