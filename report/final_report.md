...
The collected fuzzing results show that:

- on Windows, with the fixed seed `20260607`, 1000 generation-based cases were executed and no round-trip or same-process stability failure was found
- on Linux, with the same fixed seed, 2000 generation-based cases were executed and no round-trip or same-process stability failure was found
- lexical fuzzing produced 30 mutated byte streams in each environment; Windows loaded 9 mutated streams and Linux loaded 10 mutated streams, while the remaining cases raised controlled exceptions
...
The representative source-to-test mapping is summarized in this section and in the traceability matrix. The numeric statement-coverage example is recorded in the statement coverage result files.
...
Compatibility Testing checks whether the same test suite still passes under different execution environments. The bundled evidence currently covers Windows and Linux:

- Windows local environment: Python 3.9.15. The unit test suite passed, the statement-coverage example passed, and generation-based fuzzing reported 0 failures.
- Linux server environment: Python 3.10.12 on Linux 5.15. The same unit test suite passed, the statement-coverage example passed, and generation-based fuzzing reported 0 failures.

The comparison focuses on whether round-trip correctness, exception handling, boundary-value tests, same-process dump stability, and fuzzing checks remain consistent. Since `marshal` is an internal format and is not guaranteed to remain byte-stable across Python versions, version-level byte differences should be treated as compatibility evidence rather than automatic defects.

The current project has bundled Windows and Linux evidence. Additional runs on macOS and more Python versions, such as Python 3.11, 3.12, and 3.13, can further strengthen the compatibility evidence.
...
| --- | --- | --- | --- | --- |
| Stability verification: identical input produces identical output | `dict_string_keys` | `int_256`, `float_negative_zero` | generated nested containers | Windows and Linux result files |
...
| Compound structure processing | `tuple_nested`, `dict_nested` | `list_large`, `dict_large` | nested random containers | Windows and Linux repeated runs |
| Special structure processing | `recursive_list`, `recursive_dict`, `shared_reference_list` | indirect recursion | generated nested recursion-free structures | Windows and Linux repeated runs |
| Extreme value processing | representative integer / float classes | `2**63`, `NaN`, `-0.0` | random large-range numbers | Windows and Linux repeated runs |
| Empty structure processing | `list_empty`, `dict_empty`, `bytes_empty` | empty values | random empty containers | same OS repeated runs |
| Deep nesting processing | `list_nested`, `dict_nested` | large nested cases | recursive grammar generation | Windows and Linux repeated runs |
| Invalid input handling | unsupported function / file handle | truncated stream | mutated byte streams | Windows and Linux repeated runs |
| Statement Coverage example | `_register_pair()` first visit | `_register_pair()` revisit | not applicable | `results-windows/statement_coverage.md` and `results-rag-linux/statement_coverage.md` |
...
1. The bundled compatibility evidence currently covers Windows with Python 3.9.15 and Linux with Python 3.10.12; macOS and additional Python versions such as 3.11, 3.12, and 3.13 have not yet been fully bundled.
...
- bundled compatibility evidence currently covers Windows and Linux, and broader operating-system / Python-version coverage remains future work
